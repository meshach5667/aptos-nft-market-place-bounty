...
address 0x1630fd1f7f0c18995b46c243eaaa8ab04e9a919dae2d2ea94acff2825f0d9014 {
    ...
    module NFTMarketplace {
        ...
        // TODO# 21: Implement Rarity System for NFTs
        #[view]
        public fun get_nft_rarity(marketplace_addr: address, nft_id: u64): u8 acquires Marketplace {
            let marketplace = borrow_global<Marketplace>(marketplace_addr);
            let nft = vector::borrow(&marketplace.nfts, nft_id);
            nft.rarity
        }

        // TODO# 22: Marketplace Fee Deduction during Purchase
        public entry fun purchase_nft_with_fee(account: &signer, marketplace_addr: address, nft_id: u64, payment: u64) acquires Marketplace {
            let marketplace = borrow_global_mut<Marketplace>(marketplace_addr);
            let nft_ref = vector::borrow_mut(&mut marketplace.nfts, nft_id);
        ...
        // TODO# 23: Implement Advanced Rarity-based Sale Listings
        #[view]
        public fun get_rare_nfts_for_sale(marketplace_addr: address, rarity_threshold: u8): vector<u64> acquires Marketplace {
            let marketplace = borrow_global<Marketplace>(marketplace_addr);
            let nft_ids = vector::empty<u64>();
            ...
            while (mut_i < nfts_len) {
                let nft = vector::borrow(&marketplace.nfts, mut_i);
                if (nft.for_sale && nft.rarity >= rarity_threshold) {
                    vector::push_back(&mut nft_ids, nft.id);
                };
                mut_i = mut_i + 1;
            };
        ...
        // TODO# 24: Transfer NFT to Another Wallet
    public entry fun transfer_nft_to_wallet(account: &signer, marketplace_addr: address, nft_id: u64, recipient: address) acquires Marketplace {
    let marketplace = borrow_global_mut<Marketplace>(marketplace_addr);
    let nft_ref = vector::borrow_mut(&mut marketplace.nfts, nft_id);

    assert!(nft_ref.owner == signer::address_of(account), 300); // Caller is not the owner
    assert!(nft_ref.owner != recipient, 301); // Prevent transfer to the same owner
...
public entry fun transfer_nft(
    ...
    let marketplace = borrow_global_mut<Marketplace>(marketplace_addr);
    let nft_ref = vector::borrow_mut(&mut marketplace.nfts, (nft_id as u64)); // Cast to u64 if your vector index needs it
   ...
   //TODO #26: Define NFTSaleInfo struct for tracking sales data
    struct NFTSaleInfo has copy, drop {
        name: vector<u8>,
        sales: u64,
        revenue: u64
    }
// TODO# 27: Implement Dashboard Analytics Function
#[view]
public fun get_marketplace_analytics(marketplace_addr: address): (u64, vector<NFTSaleInfo>) acquires Marketplace {
    let marketplace = borrow_global<Marketplace>(marketplace_addr);
    
    // Initialize tracking variables
    let total_sales = 0u64;
    let popular_nfts = vector::empty<NFTSaleInfo>();
    let total_sales_value = 0u64;  // Additional analytics: total sales value

    // Track sales data for each NFT
    let nfts_len = vector::length(&marketplace.nfts);
    let mut_i = 0;
    ...
    while (mut_i < nfts_len) {
        ...
        // Only count NFTs that have been sold (price = 0 and not for sale indicates a completed sale)
        if (!nft.for_sale && nft.price == 0) {
            total_sales = total_sales + 1;

            // Add to total sales value
            let sale_price = nft.price;
            total_sales_value = total_sales_value + sale_price;

            // Create sale info for this NFT
            let sale_info = NFTSaleInfo {
                name: nft.name,
                sales: 1,
                revenue: sale_price
            };
            vector::push_back(&mut popular_nfts, sale_info);
        };
        mut_i = mut_i + 1;
    };

    // Return total sales and other analytics (without total revenue and avg price)
    (total_sales, popular_nfts)
}
